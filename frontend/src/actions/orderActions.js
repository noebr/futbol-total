...
const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data: { data: newOrder } } = await Axios.post(URL_SERVER+"/api/orders", order, {
      headers: {
...
const listMyOrders = () => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get(URL_SERVER+"/api/orders/mine", {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const listOrders = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get(URL_SERVER+"/api/orders", {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const payOrder = (order, paymentResult) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.put(URL_SERVER+"/api/orders/" + order._id + "/pay", paymentResult, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const deleteOrder = (orderId) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.delete(URL_SERVER+"/api/orders/" + orderId, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
